 ...
 *
 * Spins up a Java helper subprocess on first tool call, exposes 29 tools
...
- أداة get_tafseer_of_aya تعتمد فهرسًا منتقًى لا يشمل كل التفاسير المنزَّلة؛ لاستيفائها راجع تصنيفات التفسير المنزَّلة عبر shamela_list_downloaded_books(category_id=3 ثم 4)، ولا تَعُدَّ غيابَ كتابٍ من القائمة دليلًا على أنه لا يفسّر الآية. ولبيان تغطية تفاسيرك المنزَّلة لآيةٍ بعينها استعمل shamela_list_tafsirs_for_aya (تُرجع لكل كتاب حالته الثلاثية: مفهرس ويشمل الآية / مفهرس بلا مدخل لها / غير مفهرس والتغطية غير معلومة)، ولجلب نصوص التفسير المفهرسة من عدة مصادر دفعةً واحدة استعمل shamela_get_tafseer_texts.`;
...
/**
 * Build the MCP server with all 29 tools registered. The `getBackend` callback
...
async function main(): Promise<void> {
    ...
    logInfo(`shamela-mcp v${VERSION} ready (29 tools + 4 resources + 6 prompts registered)`);
