...
- إذا سأل المستخدم عمّا تستطيعه الإضافة أو كيف يستعملها فاعرض جولة موجزة من مورد shamela://guide.`;
...
export function createServer(getBackend: () => Promise<Backend>): McpServer {
    ...
    server.registerResource(
        "guide",
        "shamela://guide",
        { title: "دليل استخدام الإضافة", description: "دليل عربي للمستخدم: الأدوات التسع والعشرون بأمثلة طلبات طبيعية، والقوالب الستة، ونصائح الباحث.", mimeType: "text/markdown" },
...
async function main(): Promise<void> {
    ...
    logInfo(`shamela-mcp v${VERSION} ready (29 tools + 5 resources + 6 prompts registered)`);
