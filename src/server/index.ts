 ...
 */
// Server instructions live in the language catalogue — see src/server/i18n/.

/** Developer-facing data-model summary, exposed as the shamela://schema resource. */
// The data-model summary lives in the language catalogue too.
...
export function createServer(getBackend: () => Promise<Backend>): McpServer {
    ...
    server.registerTool(
        "shamela_get_page_services",
        {
            ...
            description:
                "Read the per-page services annotations (Qur'anic verses cited, hadith keys, isnād chains) for a specific (book_id, page_id). Returns has_services flag plus three arrays: ayat (cumulative aya_ids), hadeeth (hadith keys), esnad (chain strings). Many books — particularly non-hadith works — have no services and return has_services:false cleanly. Useful to pivot from a search hit to the Qur'anic/hadith content it discusses: pair the returned aya_ids with shamela_get_aya, or hadith keys with shamela_get_books_for_hadith. The `esnad` entries are NOT names and NOT author_ids: each is Shamela's own chain string — a whitespace-separated run of numeric narrator ids from its narrator table (service/S1.db, 18,989 narrators). That is a different id space from the ~3,200 BOOK authors shamela_search_authors and shamela_get_author read out of master.db, and the two collide silently: narrator 1596 is حكيم بن حزام (d. 50 AH), while shamela_get_author({author_id:1596}) returns حمد بن حمدي الصاعدي, a contemporary writer, with no error and no warning. No tool here resolves a narrator id, so report these ids AS ids and never render one as a name. Only musnad hadith works carry esnad at all — on a library without them the array is always empty.",
    ...
    server.registerTool(
        "shamela_search_hadith",
        {
            ...
            description:
                "Find a hadith by its TEXT (not its numeric key). Text-searches the downloaded library (matn + footnotes), reads each matching page's service annotations for hadith keys, then resolves each key's cross-collection takhrij via hadeeth.db. Returns matched pages (snippets often show the printed takhrij «رواه البخاري ومسلم») plus cross-book takhrij where service keys exist. Note: fiqh/usul libraries frequently lack service keys on cited-hadith pages — the snippets still carry the printed takhrij. For takhrij work, search options.search_in:['foot'] alone to target editors' footnotes, where the printed referencing lives. Chains: shamela_get_page_services returns a page's `esnad` array, but its entries are numeric narrator ids from Shamela's narrator table (service/S1.db), not names — they are NOT author_ids, no tool here resolves them, and feeding one to shamela_get_author returns a different person with no warning (narrator 1596 is حكيم بن حزام; author 1596 is a contemporary writer). Report such ids as ids. Example: shamela_search_hadith({query:'إنما الأعمال بالنيات'}).",
    ...
    server.registerTool(
        "shamela_search_exact",
        {
            ...
            description:
                "Exactness-preserving search the regular search cannot do: it honors diacritics (التشكيل), hamza/alef forms (ٱآأإ vs bare ا, plus ؤ ئ ء ى ة), and digit systems (Arabic-Indic ٠-٩ vs Western 0-9). shamela_search_pages folds all of these away (preserve_* return OPTION_NOT_SUPPORTED). Two-stage, no index change: (1) normalized AND-search gathers candidates; (2) each candidate's FULL stored page text is re-read and verified in Node, folding ONLY the features you did NOT ask to preserve — that text comes from the page index's STORED fields (the same source shamela_get_page reads); the per-book SQLite holds no text at all, only page(id, part, page, number, services). Type the query WITH the diacritics/hamza/digits to enforce; enable at least one flag in `preserve`. Broad searches may miss matches outside the bounded candidate window (`candidate_cap_hit`/`total_candidates_scanned` report it). That window is min(max(limit×8, 40), 100) candidate pages — 40 at limit ≤ 5, 100 at the default limit — and `scope` does NOT enlarge it: scoping to a single book still stops at the same ceiling, it only decides which corpus the window is drawn from. When `candidate_cap_hit` is true the answer is partial: tighten the query to rarer words, or narrow the scope, and do not read the result as absence. Examples: shamela_search_exact({query:'أحمد', preserve:{preserve_hamza:true}}) won't match «احمد»; shamela_search_exact({query:'عِلْم', preserve:{preserve_diacritics:true}}) won't match «عَلَم».",
