...
/** EXPERIMENTAL — distinguishes the catalog flag from real readable content (Bug 6). */
...
export interface GetBookOutput {
    ...
    available: boolean;
    downloaded: boolean;
    /** EXPERIMENTAL (Bug 6): tri-state content availability, distinct from the catalog `downloaded` flag. */
    content_status: ContentStatus;
    authors: AuthorEntry[];
    pdf_links: string | null;
    publication_date: string | null;
    /** EXPERIMENTAL (Proposal 18): edition/publisher hint from Shamela's name suffix (e.g. «ت التركي»). */
    edition: string | null;
    /** EXPERIMENTAL (Proposal 18): muḥaqqiq, conservatively extracted from the book's front-matter (null if not clearly found). */
    editor: string | null;
    /** EXPERIMENTAL (Proposal 18): publisher, conservatively extracted from the book's front-matter (null if not clearly found). */
...
export async function runGetBook(
    ...
    // Bug 6: master.db.book.major_ondisk can flip true while the per-book SQLite
    ...
    // Proposal 18: edition / muḥaqqiq / publisher. Shamela encodes these in the
